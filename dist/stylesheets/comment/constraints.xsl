<xsl:stylesheet
    ...
    <xsl:template name="blacklist">
        <xsl:param name="url"/>
        <!-- Strip the URL down to just the domain -->
        <xsl:variable name="url" select="weblog:normalizeURL($url)"/>
        
        <!-- Make sure it's not already on the list. -->
        <xsl:variable name="query">/blacklist[url = '<xsl:value-of select="$url"/>']</xsl:variable>
        <xsl:if test="not(dbxsl:xpathQuery($config, $query))">
            <!-- Add it to the list -->
            <xsl:variable name="entry">
                <blacklist><url><xsl:value-of select="$url"/></url></blacklist>
            </xsl:variable>
